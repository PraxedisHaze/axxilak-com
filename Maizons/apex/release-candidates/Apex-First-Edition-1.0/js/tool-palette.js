                ...
                <div class="palette-content">
                    ...
                    <div class="palette-section">
                        ...
                        <div id="layout-panel" class="palette-section-content" role="region" aria-labelledby="layout-toggle">
                            ...
                            <!-- Structure & Hierarchy -->
                            <div class="palette-control mt-5 pt-4 border-t border-zinc-800">
                                <label class="palette-label text-[9px] mb-1">Arrange this item</label><p class="palette-help">These change its place among neighboring items. They do not change the words inside it.</p><button id="btn-select-parent" type="button" class="palette-action-button w-full mt-3 mb-2 px-2 py-2 text-[9px] font-bold uppercase tracking-widest rounded transition">Edit containing box</button><p class="palette-help mb-3">Selects the immediate box around the selected item. Apex will ask before changing an unsaved edit.</p>
                                <div class="grid grid-cols-2 gap-2">
                                    <button id="btn-move-up" class="palette-layout-button px-2 py-2 rounded transition ${isLocked ? 'opacity-30' : ''}" ${isLocked ? 'disabled' : ''} aria-label="${isHorizontal ? 'Move element left' : 'Move element up'}">${isHorizontal ? 'Left' : 'Up'}</button>
                                    <button id="btn-move-down" class="palette-layout-button px-2 py-2 rounded transition ${isLocked ? 'opacity-30' : ''}" ${isLocked ? 'disabled' : ''} aria-label="${isHorizontal ? 'Move element right' : 'Move element down'}">${isHorizontal ? 'Right' : 'Down'}</button>
                                    <button id="btn-clone" class="palette-layout-button px-2 py-2 rounded transition ${isLocked ? 'opacity-30' : ''}" ${isLocked ? 'disabled' : ''} aria-label="Copy element">Copy</button>
                                    <button id="btn-delete" class="palette-layout-button palette-delete-button px-2 py-2 rounded transition ${isLocked ? 'opacity-30' : ''}" ${isLocked ? 'disabled' : ''} aria-label="Delete element">Delete</button>
                                </div>
                            </div>
                        </div>
                    </div>
