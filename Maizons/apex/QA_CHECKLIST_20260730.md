...
- [x] Mobile menu toggle (375px) — FIXED 2026-08-04: retired the duplicate `attachHandlers()` binder, leaving `HandlerDispatcher` as the sole `data-handler` authority. Real hit-test proof: `document.elementFromPoint(348,32)` returned the actual Open Menu button and a physical down/up at that point left the menu open. No direct element dispatch used.
- [x] Edge iframe is deferred until Edge opens — FIXED 2026-08-04: its initial `src` is absent on a fresh Apex load, avoiding startup execution of the remote Edge app. An observed remote `demo_photo.png` request is not a functional dependency: Edge Electrify runs normally without that photo.
- [x] Media URL markup safety — FIXED 2026-08-04: direct-video embedding now uses DOM-created media nodes and allow-listed URLs rather than raw `innerHTML`; browser sanity check rejected `javascript:` and encoded a quote-bearing payload.
...
- [x] Snake mini-game is usable on phone — FIXED 2026-08-04: board scales to the viewport, touch D-pad offers Up/Left/Down/Right, and a fixed 44px close button stays reachable at top-right. Phone-size modal layout and real-hit-test close click verified.
...
- [x] Mobile Theme and EDIT controls are reachable — FIXED 2026-08-04: both are now in the always-visible mobile header beside the hamburger, after the first dropdown placement proved unreadable in dark mode. At 375px, real hit-tests followed by physical clicks changed theme and entered Edit Mode; mobile EDIT reports its pressed state. The overlay and handler allow-list share the same live-control definition so EDIT remains a valid exit route after selection.
- [x] Mobile editor leaves selectable page space — FIXED 2026-08-04: ToolPalette's desktop inline geometry overrode the prior responsive height cap, covering nearly all of a 720px viewport. The mobile palette is now a 42vh bottom sheet; after selection it measured 302px high and left 406px of live page above. Real hit-test followed by physical click selected the hero heading.
