export class ToolPalette {
    ...
    matchElementStyles(element, styles) {
        ...
        if (styles.color) this.quill.root.style.color = styles.color;
        if (styles.backgroundColor && styles.backgroundColor !== 'rgba(0, 0, 0, 0)')
            this.quill.root.style.backgroundColor = styles.backgroundColor;
        if (styles.fontSize) this.quill.root.style.fontSize = styles.fontSize;
        if (styles.fontFamily) this.quill.root.style.fontFamily = styles.fontFamily;
        if (styles.fontWeight) this.quill.root.style.fontWeight = styles.fontWeight;
        if (styles.lineHeight) this.quill.root.style.lineHeight = styles.lineHeight;
    }
    ...
    attachListeners() {
        ...
        if (linkUrlInput) {
            const applyLinkUrl = () => {
                if (this.onEdit) this.onEdit('href', linkUrlInput.value.trim());
            };
            linkUrlInput.onchange = applyLinkUrl;
            linkUrlInput.addEventListener('keydown', (e) => {
                if (e.key === 'Enter') applyLinkUrl();
            });
        }
